...
async function MemoryStats() {
  const response = await fetch(`${process.env.MEMORY_SERVER_URL || 'http://localhost:4100'}/memory/mcp`, {
    method: 'POST',
    headers: {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900">
        ...
        <CardContent>
          <div className="text-2xl font-bold">{stats.storage?.total_memories || 0}</div>
      ...
      <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-950 dark:to-green-900">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Graph Concepts</CardTitle>
          <Network className="h-4 w-4 text-green-600" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{stats.graph?.length || 0}</div>
          <p className="text-xs text-muted-foreground">Connected memory clusters</p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-950 dark:to-amber-900">
        ...
        <CardContent>
          <div className="text-2xl font-bold">{Object.keys(stats.storage?.by_agent || {}).length}</div>
          <p className="text-xs text-muted-foreground">Unique memory contributors</p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-950 dark:to-purple-900">
        ...
        <CardContent>
          <div className="text-2xl font-bold">{stats.storage?.recent_activity?.length || 0}</div>
          <p className="text-xs text-muted-foreground">Last 10 memories</p>
        </CardContent>
      </Card>
    </div>
  )
